...
# Модель для запроса на исправление кода
    ...
    <script>
        ...
        async function runAudit() {
            ...
            try {
                ...
                // Если мы скачали код с гитхаба, запоминаем его для функции исправления
        ...
        // Новая JS функция для вызова ручки исправления кода
        async function fixVulnerabilities() {
            ...
            try {
                ...
                // Скроллим экран вниз к готовому решению
...
def analyze_security(data: CodeAnalysisInput):
    ...
    return {
        ...
        "extracted_code": final_code # Возвращаем код для JS контекста
    }

# Новый API эндпоинт для авто-исправления кода
@app.post("/api/v1/fix-security")
def fix_security(data: CodeFixInput):
    openai_key = os.getenv("OPENAI_API_KEY")
    
    if not openai_key:
        # Умная демонстрационная заглушка безопасного кода
        return {
            "status": "patched",
            "fixed_code": (
                "import os\n"
                ...
                "import requests\n\n"
                "def connect_to_db():\n"
                "    # [SHIELD-AI PATCHED] Hardcoded password removed. Using environment variables.\n"
                "    db_password = os.getenv('DATABASE_SECURE_PASSWORD')\n"
                "    print('Connecting securely with env-driven credentials...')\n\n"
                "def execute_query(query):\n"
                "    # [SHIELD-AI PATCHED] Fixed SQL/Shell Injection risk. Removed os.system().\n"
                "    # Using secure subprocess array sequence with shell execution disabled.\n"
        ...
        try:
            ...
            prompt = f"You are a Senior DevSecOps Engineer. Fix all critical security issues in this Python code based on the security report provided. Return ONLY the clean, ready-to-use, secure Python code. Do not include markdown formatting, backticks like ```python, or explanations. Just pure code:\n\nCode:\n{data.vulnerable_code}\n\nReport:\n{data.analysis_report}"
            ...
            response = requests.post("[https://api.openai.com/v1/chat/completions](https://api.openai.com/v1/chat/completions)", json=payload, headers=headers)
            return {
                "status": "patched",
...
Чтобы ShieldAI вызывал у англоязычных разработчиков непреодолимое желание достать карту и оплатить подписку, продукту нужна **киллер-фича**. Сейчас сканер находит уязвимости, но оставляет пользователя один на один с проблемой. 

Давай добавим функцию, за которую на западном рынке платят больше всего — **Автоматическое исправление кода одной кнопкой (AI-Powered Auto-Remediation)**.

Представь: твой сервис не просто пугает разработчика отчетом, а предлагает кнопку: **«Fix with ShieldAI»**. Пользователь нажимает её, и система выдает ему идеальный, безопасный код, который можно сразу скопировать и вставить вместо уязвимого.

---

### Что конкретно мы добавим в код прямо сейчас:

1. **Интерактивная кнопка исправления (`Fix Code via AI`)**:
   Она появится прямо под отчетом об уязвимостях.
2. **Новый роут на бэкенде (`/api/v1/fix-security`)**:
   FastAPI будет принимать уязвимый код и отчет сканера, отправлять их ИИ с жестким промптом: *«Выдай только исправленный, безопасный код без лишней болтовни»*.
3. **Окно с готовым решением (Safe Code Output)**:
   На фронтенде появится красивое зеленое окно с чистым кодом, защищенным от хакерских атак.

Вот полностью готовый, усиленный код для твоего файла `main.py`. Замени им старую версию на GitHub:
```python
import os
import subprocess
import re
